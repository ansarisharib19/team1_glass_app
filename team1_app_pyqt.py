...
API_URL = "https://script.google.com/macros/s/AKfycbxyz123/exec"
...
if st.button("➕ ADD REQUEST", use_container_width=True):
    ...
    # ---- SHOW PREVIEW ----
    preview_df = pd.DataFrame([new_row])
    st.subheader("🔍 Preview – Newly Added Request")
    st.dataframe(preview_df, use_container_width=True)

    # ---- DOWNLOAD ----
    st.download_button(
        label="⬇️ Download This Request",
        data=preview_df.to_csv(index=False).encode(),
        file_name=f"Request_{request_number}.csv",
        mime="text/csv"
    )
